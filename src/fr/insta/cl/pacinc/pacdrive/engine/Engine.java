...
public class Engine implements EngineService, RequireDataService, RequireAlgorithmService {
    ...
    private AlgorithmService iaAlgorithme;
...
//        }
                ...
                for (HostileService h : data.getHostiles()) {
                    ...
                    switch (h.getComportement()) {
                        case "avancee1":
                            ...
                            iaAlgorithme.iaAvancee1(h);
                            break;
                        case "avancee2":
                            iaAlgorithme.iaAvancee2(h);
                            break;
                        default:
                            iaAlgorithme.iaPrimitive(h);
                    }
    ...
    private void updateSpeedHeroes() {
        data.getJoueur().getVitesse().x *= friction;
        data.getJoueur().getVitesse().y *= friction;
    }

    private void updateCommandHeroes() {
        if (moveLeft) data.getJoueur().getVitesse().x -= heroesStep;
        if (moveRight) data.getJoueur().getVitesse().x += heroesStep;
        if (moveUp) data.getJoueur().getVitesse().y -= heroesStep;
        if (moveDown) data.getJoueur().getVitesse().y += heroesStep;
        if (putMine) {
    ...
    private void updatePositionHeroes() {
        data.setHeroesPosition(new Position(data.getHeroesPosition().x + data.getJoueur().getVitesse().x, data.getHeroesPosition().y + data.getJoueur().getVitesse().y));
    ...
    public boolean collisionBetweenPositionnables(PositionnableService object1, PositionnableService object2) {
        ...
        if ((p1.x + p1Width > p2.x && p1.x + p1Width < p2.x + p2Width && p1.y > p2.y && p1.y < p2.y + p2Height))
            return true;
        if ((p1.x + p1Width > p2.x && p1.x + p1Width < p2.x + p2Width && p1.y + p1Height > p2.y && p1.y + p1Height < p2.y + p2Height))
            return true;
        if ((p1.x > p2.x && p1.x < p2.x + p2Width && p1.y + p1Height > p2.y && p1.y + p1Height < p2.y + p2Height))
            return true;

        if ((p2.x > p1.x && p2.x < p1.x + p1Width && p2.y > p1.y && p2.y < p1.y + p1Height)) return true;
        if ((p2.x + p2Width > p1.x && p2.x + p2Width < p1.x + p1Width && p2.y > p1.y && p2.y < p1.y + p1Height))
            return true;
        if ((p2.x + p2Width > p1.x && p2.x + p2Width < p1.x + p1Width && p2.y + p2Height > p1.y && p2.y + p2Height < p1.y + p1Height))
            return true;
        if ((p2.x > p1.x && p2.x < p1.x + p1Width && p2.y + p2Height > p1.y && p2.y + p2Height < p1.y + p1Height))
            return true;
